              ...
              <tbody className="text-slate-700 text-sm divide-y divide-slate-50">
                ...
                {filtered.map((product) => (
                  <tr key={product._id} className="hover:bg-slate-50/50 transition-colors">
                    <td className="p-4 text-xs text-indigo-500 font-mono font-semibold">{product.productId}</td>
                    <td className="p-4 font-medium text-slate-900 max-w-[200px] truncate">{product.name}</td>
                    <td className="p-4 text-slate-500">{product.category}</td>
                    <td className="p-4">
                      <span className="font-semibold">{product.currency} {product.price}</span>
                      {product.discountPrice != null && (
                        <span className="ml-2 text-xs text-green-600">{product.currency} {product.discountPrice}</span>
                      )}
                    </td>
                    <td className="p-4">
